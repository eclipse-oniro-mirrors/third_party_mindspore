...
class MS_API Model {
 public:
  ...
  /// \brief Set model aipp predict parameter.
  /// \return void.
  void SetPredictModelPara(const char *predict_config);

  /// \brief Check if using aipp config in model.
  /// \return true if using aipp config in model.
  bool GetUseAipp();
