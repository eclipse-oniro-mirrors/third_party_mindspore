...
class MS_API MSKernel {
 public:
  ...
  /// \brief set model aipp predict config.
  ///
  /// \param[in] config model aipp predict config.
  void SetPredictParaKernel(const char *predict_config)
  {
    predict_config_ = predict_config;
  }
