...
int BenchmarkBase::CheckDeviceTypeValid() {
  if (flags_->device_ != "CPU" && flags_->device_ != "GPU" && flags_->device_ != "NPU" &&
      flags_->device_ != "Ascend" && flags_->device_ != "NNRT") {
...
int BenchmarkBase::Init() {
  ...
  MS_LOG(INFO) << "PerformanceMode = " << this->flags_->nnrt_performance_mode_;
  ...
  std::cout << "PerformanceMode = " << this->flags_->nnrt_performance_mode_ << std::endl;
