...
namespace tools {
namespace {
constexpr size_t kNPUPrefixLen = 4;
}
...
int BenchmarkCApi::RunBenchmark() {
  ...
  if (!flags_->config_file_.empty()) {
    ret = OH_AI_ModelLoadConfig(model_, flags_->config_file_.c_str());
    if (ret != OH_AI_STATUS_SUCCESS) {
      BENCHMARK_LOG_ERROR("OH_AI_ModelLoadConfig failed, ret: " << ret);
    }
  }
  ...
  MS_LOG(INFO) << "PrepareTime = " << ((end_prepare_time - start_prepare_time) / kFloatMSEC) << " OH_AI_";
  std::cout << "PrepareTime = " << ((end_prepare_time - start_prepare_time) / kFloatMSEC) << " OH_AI_" << std::endl;
...
int BenchmarkCApi::InitContext() {
  ...
  if (flags_->device_ == "NNRT") {
    ...
    NNRTDeviceDesc *desc_nnrt = nullptr;
    for (size_t i = 0; i < num; i++) {
      auto desc = OH_AI_GetElementOfNNRTDeviceDescs(descs, i);
      auto name = OH_AI_GetNameFromNNRTDeviceDesc(desc);
      if (strncmp(name, "NPU_", kNPUPrefixLen) == 0) {  // npu推理，在线编译
        desc_nnrt = desc;
        break;
      }
    }
    if (desc_nnrt == nullptr) {
      BENCHMARK_LOG_ERROR("nnrt desc get failed");
      return RET_ERROR;
    }
    auto id = OH_AI_GetDeviceIdFromNNRTDeviceDesc(desc_nnrt);
...
int BenchmarkCApi::MarkPerformance() {
  ...
  if (flags_->loop_count_ > 0) {
    ...
    printf("Model = %s, NumThreads = %d, MinRunTime = %f OH_AI_, MaxRuntime = %f OH_AI_, AvgRunTime = %f OH_AI_\n",
           flags_->model_file_.substr(flags_->model_file_.find_last_of(lite::DELIM_SLASH) + 1).c_str(),
