...
int NetTrainCApi::SaveModels() {
  if (!flags_->export_file_.empty()) {
    ...
    auto status = OH_AI_ExportModel(ms_model_, OH_AI_MODELTYPE_MINDIR, (flags_->export_file_).c_str(), OH_AI_NO_QUANT,
                                    false, nullptr, 0);
    if (status != OH_AI_STATUS_SUCCESS) {
  ...
  if (!flags_->inference_file_.empty()) {
    ...
    auto tick = GetTimeUs();
    status = OH_AI_ExportModel(ms_model_, OH_AI_MODELTYPE_MINDIR, (flags_->inference_file_).c_str(), OH_AI_NO_QUANT,
                               true, nullptr, 0);
    if (status != OH_AI_STATUS_SUCCESS) {
