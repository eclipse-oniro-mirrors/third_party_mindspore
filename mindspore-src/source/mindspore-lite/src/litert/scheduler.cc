...
constexpr int kWhereInputNum = 3;
...
int Scheduler::ReplaceDelegateKernels(std::vector<kernel::KernelExec *> *dst_kernels) {
  ...
  if (context_->IsDeviceTypeEnabled(DT_NNRT)) {
    ...
    delegate->xpu_backend_set_ = xpu_backend_set_;
    if (hete_execution_plan_ != nullptr && !hete_execution_plan_->empty()) {
      delegate->enable_hete_ = true;
      delegate->hete_execution_plan_ = hete_execution_plan_;
      delegate->original_hete_execution_plan_ = original_hete_execution_plan_;
      MS_LOG(INFO) << "hete_execution_plan_ is valid and enable heterogeneous inference in Delegate. "
                   << "hete_execution_plan_ size:" << hete_execution_plan_->size();
    } else {
      MS_LOG(INFO) << "Disable heterogeneous inference in Delegate.";
    }
  }
#endif
...
int Scheduler::InferSubGraphShape(size_t subgraph_index) {
  ...
  for (size_t i = 0; i < node_indexes.size(); ++i) {
    ...
    if (ret == RET_INFER_INVALID) {
      MS_LOG(DEBUG) << "InferShape interrupted, name: " << node->name_
...
kernel::KernelExec *Scheduler::FindBackendKernel(const std::vector<Tensor *> &in_tensors,
  ...
  if (node->quant_type_ == schema::QuantType_QUANT_WEIGHT) {
    ...
    if (static_cast<std::string>(GetPrimitiveTypeName(node->primitive_, context_->get_schema_version())) == "Where" &&
        in_tensors.size() == kWhereInputNum) {
      // mslite use first input tensor dtype as kernel dtype, but op 'where' is a outliner, is first input 'condition'
      // is always 'bool'-typed, here we use where's second/third input dtype as kernel dtype.
      ...
      if (type_set.find(in_tensors[kIndex1]->data_type()) != type_set.end()) {
        data_type = in_tensors[kIndex1]->data_type();
      } else {
        data_type = in_tensors[kIndex2]->data_type();
      }
    } else {
