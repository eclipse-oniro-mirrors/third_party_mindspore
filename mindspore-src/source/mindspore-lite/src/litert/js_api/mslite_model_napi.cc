...
constexpr bool ShouldUseDefaultCpuContext(bool has_target, bool has_cpu, bool has_nnrt) {
  return !has_target && !has_cpu && !has_nnrt;
}
...
int32_t MSLiteModelNapi::ParseContextInfo(napi_env env, napi_value args, ContextInfo &context) {
  ...
  bool has_cpu = false;
  bool has_nnrt = false;
  status = napi_has_named_property(env, args, "target", &has_target);
  if (status != napi_ok) {
    MS_LOG(ERROR) << "Get context target property state failed.";
    return ERR_INVALID_OPERATION;
  }
  status = napi_has_named_property(env, args, "cpu", &has_cpu);
  if (status != napi_ok) {
    MS_LOG(ERROR) << "Get context cpu property state failed.";
    return ERR_INVALID_OPERATION;
  }
  status = napi_has_named_property(env, args, "nnrt", &has_nnrt);
  if (status != napi_ok) {
    MS_LOG(ERROR) << "Get context nnrt property state failed.";
    return ERR_INVALID_OPERATION;
  }
  if (ShouldUseDefaultCpuContext(has_target, has_cpu, has_nnrt)) {
    return SUCCESS;
  }
...
napi_value MSLiteModelNapi::LoadMSLiteModelFromFile(napi_env env, napi_callback_info info) {
  ...
  if (asyncContext->callbackRef == nullptr) {
    if (asyncContext->status != SUCCESS) {
        MS_LOG(ERROR) << "Parsing args failed.";
        return result;
    }
