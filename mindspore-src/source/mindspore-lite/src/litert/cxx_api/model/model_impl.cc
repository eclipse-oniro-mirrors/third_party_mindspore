...
void ModelImpl::SetPredictPara(const char *predict_config) { predict_config_ = predict_config; }
...
Status ModelImpl::RunGraph(const MSKernelCallBack &before, const MSKernelCallBack &after) {
  ...
  session_->SetPredictParaSession(predict_config_);
...
Status ModelImpl::LoadConfig(const std::string &config_path) {
  ...
  auto parse_ret = lite::ParserMultiExecutionPlan(&config_info, &execution_plan_, &hete_execution_plan_);

  if (parse_ret != RET_OK) {
    MS_LOG(ERROR) << "LoadConfig failed: please use 'backend:CPU' only";
    xpu_backend_set_ = false;
    return kLiteError;
  }
  original_hete_execution_plan_ = hete_execution_plan_;
  config_info_ = all_config_info;
  MS_LOG(DEBUG) << "LoadConfig succeeded - loaded " << hete_execution_plan_.size()
               << " heterogeneous execution plan(s)";
  return kSuccess;
}
...
Status ModelImpl::Predict(const std::vector<MSTensor> &inputs, std::vector<MSTensor> *outputs,
  ...
  for (size_t i = 0; i < inputs_size; i++) {
    ...
    if (predict_config_ == nullptr && user_input.DataType() != static_cast<enum DataType>(input->data_type())) {
    ...
    if (input->data_type() == kObjectTypeString) {
      ...
      if (user_input.MutableData() != input->data()) {
        if (predict_config_ == nullptr && input->Size() != user_input.DataSize()) {
          ResetTensorData(old_data, input_tensors);
        ...
        if (user_input.allocator() == input->allocator()) {
          ...
          if (user_input.DataSize() != input->Size()) {
            MS_LOG(ERROR) << "The size of the user-provided input data does not match the modle's expected input size";
            return kLiteInputParamInvalid;
          }
          memcpy(input_data, user_data, input->Size());
        }
      } else if (predict_config_ != nullptr) {
        MS_LOG(ERROR) << "Aipp must predict by create tensor!!!";
        return kLiteParamInvalid;
      }
    }
  }
  ...
  if (outputs->empty()) {
    MS_LOG(DEBUG) << "user provided output is empty";
...
lite::LiteSession *ModelImpl::CreateLiteSession(const std::shared_ptr<lite::InnerContext> &context) {
  ...
  if (iter != context->device_list_.end()) {
    ...
    if (lite::CacheSession::IsKirinNPUWithOnlineInference(nnrt_device_info.device_id_)) {
      ...
      MS_LOG(INFO) << "loaded nnrt library at OH_NNModel_HasCache";
      has_cache = nnrtWrapper.HasCache(extension_options.cache_path_.c_str(), extension_options.model_name.c_str(), extension_options.cache_version_);      
      if (has_cache) {
  ...
  session->InitHeteExecutionConfig(&hete_execution_plan_);
  session->InitOriginalHeteExecutionConfig(&original_hete_execution_plan_);
  session->SetXpuBackendSet(xpu_backend_set_);
