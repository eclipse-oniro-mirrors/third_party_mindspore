...
int LiteSession::CompileGraph(Model *model) {
  ...
  Scheduler scheduler(context_.get(), ms_context_, model, &tensors_, &inputs_, &outputs_, is_train_session_,
                      &is_infershape_, &is_control_flow_, &infer_along_running_, execution_plan_, hete_execution_plan_,
                      original_hete_execution_plan_, xpu_backend_set_,
...
#ifdef SUPPORT_NNRT
  for (auto kernel_exec : this->kernels_) {
    kernel_exec->SetPredictParaKernelExec(predict_config_);
  }
#endif
...
int LiteSession::InitSharedThreadPool() {
  ...
  MS_LOG(DEBUG) << "runner id: " << runner_id_ << "  enable_shared_pool: " << enable_shared_pool
...
#else
  MS_LOG(INFO) << "Ascend kernel plugin is not enabled in this build.";
  return RET_OK;
#endif
#else
  return RET_OK;
...
int LiteSession::InitDelegate() {
  ...
  if (delegate_ != nullptr) {
    ...
    if (delegate_ret == mindspore::kLiteNotSupport) {
      ...
      MS_LOG(DEBUG) << "Delegate init successfully";
    } else {
...
int LiteSession::Resize(const std::vector<mindspore::lite::Tensor *> &inputs,
  ...
  for (size_t i = 0; i < dims.size(); i++) {
    auto model_input_rank = inputs[i]->ConvertToTensorC()->shape_size_;
    if (dims[i].size() != model_input_rank && model_input_rank != 0) {
      MS_LOG(ERROR) << "Tensor " << i << "'s rank: " << dims[i].size() << " must match the rank: " << model_input_rank
                    << " of model input " << i;
      return RET_ERROR;
    }
  }
...
int lite::LiteSession::LoadModelAndCompileByBuf(const char *model_buf, mindspore::ModelType model_type,
                                                const size_t &buf_size) {
#endif
  MS_LOG(INFO) << "Start loading model and compiling from buffer";
  ...
  char *lite_buf = nullptr;
  MS_LOG(INFO) << "Start loading model from buffer";
  auto buf_model_type = LoadModelByBuff(new_model_buf, buf_size, &lite_buf, &lite_buf_size, model_type);
  MS_LOG(INFO) << "Successfully loaded model from buffer";
  if (buf_model_type == mindspore::ModelType::kUnknownType || lite_buf == nullptr) {
  ...
  MS_LOG(INFO) << "Start to compile Graph";
  auto ret = CompileGraph(model);
  MS_LOG(INFO) << "Has successfully compiled the graph";
  model->buf = nullptr;
  if (ret != lite::RET_OK) {
...
int lite::LiteSession::LoadModelAndCompileByPath(const std::string &model_path, mindspore::ModelType model_type) {
  MS_LOG(DEBUG) << "Start loading model and compiling from path";
  size_t model_size;
  bool use_mmap = IsMmapEnable();
  auto model_buf = LoadModelByPath(model_path, model_type, &model_size, use_mmap);
  MS_LOG(DEBUG) << "Successfully loaded model from path";
  if (model_buf == nullptr) {
  ...
  auto status = lite::PackWeightManager::GetInstance()->InitPackWeightManager(model_buf, model_size, &model_id_,
                                                                              &runner_id_, config_info_);
  MS_LOG(DEBUG) << "Successfully initialized Pack Weight Manager";
  if (status != RET_OK) {
    MS_LOG(ERROR) << "InitPackWeightByBuf failed.";
    return RET_ERROR;
  }
  ...
  MS_LOG(DEBUG) << "Successfully got the SharedModel buffer";
  ...
  MS_LOG(DEBUG) << "Start compiling the graph";
  auto ret = CompileGraph(model);
  MS_LOG(DEBUG) << "Successfully compiled the graph";
  if (ret != lite::RET_OK) {
    MS_LOG(ERROR) << "Compile model failed";
    model->buf = nullptr;
    delete model;
  ...
  if (context_->IsDeviceTypeEnabled(DT_NNRT)) {
    if (delegate_ != nullptr && reinterpret_cast<NNRTDelegate *>(delegate_.get())->IsBuildOffline()) {
        (reinterpret_cast<lite::LiteModel *>(model))->UnmapModelBuf();
    }
  }
#endif
  return RET_OK;
