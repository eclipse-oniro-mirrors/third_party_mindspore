...
// (a, b, c) -> (a, a+b, a+b+c)  exclusive == false
// (a, b, c) -> (0, a,   a+b)    exclusive == true
static inline int64_t CumsumIntOutputInitWithInputSSE(int64_t index, const int32_t *layer_input,
...
static inline int64_t CumsumIntOutputInitWithZeroSSE(int64_t index, int32_t *layer_output, int inner_dim) {
  for (int block_max_size = inner_dim - BLOCK_NUM + 1; index < block_max_size; index += BLOCK_NUM) {
...
static inline int64_t CumsumIntSSE(int64_t index, const int32_t *layer_input, int32_t *layer_output, int32_t *layer_last_output,
  int inner_dim) {
  for (int block_max_size = inner_dim - BLOCK_NUM + 1; index < block_max_size; index += BLOCK_NUM) {
...
// (a, b, c) -> (c+b+a, c+b, c) exclusive==false
// (a, b, c) -> (c+b, c, 0) exclusive==true
static inline int64_t CumsumReverseIntSSE(int64_t index, const int32_t *layer_input, int32_t *layer_output, int32_t *layer_last_output,
  int inner_dim) {
  for (int block_max_size = inner_dim - BLOCK_NUM + 1; index < block_max_size; index += BLOCK_NUM) {
