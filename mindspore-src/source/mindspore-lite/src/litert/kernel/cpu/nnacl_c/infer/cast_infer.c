...
int CastInferShape(const TensorC *const *inputs, size_t inputs_size, TensorC **outputs, size_t outputs_size,
  ...
  if (inputs_size != 2) {
    NNACL_LOG_ERROR("inputs_size != 2 ,which is %zu", inputs_size);
  ...
  if (dst_type->data_ == NULL) {
    NNACL_LOG_ERROR("dst_type->data_ == NULL");
  ...
  if (input->data_type_ != kNumberTypeBool && input->data_type_ != kNumberTypeUInt8 &&
    ...
    NNACL_LOG_ERROR("input->data_type_ should be one of [Bool,UInt8,Int8,Int32,Int64,Float32,Float16],but it is %d",
                    input->data_type_);
    return NNACL_INPUT_TENSOR_ERROR;
  }
