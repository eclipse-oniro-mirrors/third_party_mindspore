...
int RangeInferShape(const TensorC *const *inputs, size_t inputs_size, TensorC **outputs, size_t outputs_size,
  ...
  if (NNACLGetElementNum(inputs[FIRST_INPUT]) < 1) {
    NNACL_LOG_ERROR("GetElementNum(inputs[FIRST_INPUT]) < 1");
    return NNACL_ERR;
  }
  int shape_size = 0;
  if (inputs_size == C3NUM) {
    ...
    if (NNACLGetElementNum(inputs[SECOND_INPUT]) < 1 || NNACLGetElementNum(inputs[THIRD_INPUT]) < 1) {
      NNACL_LOG_ERROR("GetElementNum(inputs[SECOND_INPUT]) < 1 || GetElementNum(inputs[THIRD_INPUT]) < 1");
      return NNACL_ERR;
    }
    switch (inputs[0]->data_type_) {
      ...
      case kNumberTypeFloat: {
        ...
        if (fabsf(delta) < EPSILON_VALUE) {
          NNACL_LOG_ERROR("fabsf(delta) < EPSILON_VALUE");
          return NNACL_ERR;
        }
        shape_size = imax((int)(ceil((float)(limit - start) / delta)), 0);
      } break;
      default: {
        NNACL_LOG_ERROR("inputs[0]->data_type_ is invalid");
        return NNACL_ERR;
      }
    }
  } else {
    ...
    if (param->delta_ == 0) {
      NNACL_LOG_ERROR("param->delta_ == 0");
