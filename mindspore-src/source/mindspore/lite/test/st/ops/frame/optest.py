...
class OpTest:
    ...
    def get_run_input_shapes(self, run_config):
        ...
        assert len(input_shapes) == len(
            self.model_configs.graph_input_names[
    ...
    def gen_onnx(self):
        ...
        for op_config in op_configs:
            ...
            model = helper.make_model(
                graph, producer_name="test", opset_imports=[helper.make_opsetid("", 21)]
            )
        ...
        for gold_config in self.golden_confis.gold_configs_:
            ...
            with open(output_file, "w") as text_file:
                for i in range(len(ort_outputs)):
                    ...
                    conf.logger.info(
                        "saving output %d, %s, type is: %s, shape: %s",
                        i,
                        output_name,
                        gold_output.dtype,
                        output_shape,
        ...
        for convert_config in convert_configs:
            ...
            if disabled:
                conf.logger.debug(
                    ...
                    convert_config["out_model"],
                    convert_config["in_model"],
                )
                continue
            # 动态转动态或者静态转静态，不需要设置--inputShape参数
            input_shapes = convert_config["input_shapes"]
            self.ms_onnx_convert_reverse_dict[
                convert_config["out_model"].split(".")[0] + ".ms"
            ] = convert_config["in_model"]
            in_model_path = (
            ...
            if "SUCCESS:0" in result.stdout:
                conf.logger.info(
                    "convert model %s success", convert_config["out_model"]
                )
            else:
                conf.logger.error(
                    "convert model %s failed", convert_config["out_model"]
                )
                raise RuntimeError("convert onnx to ms model error, please check!!!")

    def run_models(self, hdc_dir=None, hdc_shell_prefix=None):
        """
        hdc_dir:手机上的算子测试工作目录,需要首先将ohos的benchmark放到该工作目录。
        hdc_shell_prefix:hdc连接的前缀,hdc -s ip:端口 -t 序列号
        """
        run_configs = self.run_configs.run_configs_
        if hdc_dir is not None and hdc_shell_prefix is not None:
            current_directory = os.path.dirname(os.getcwd())
            print("当前工作目录:", current_directory)
            hdc_push = f"{hdc_shell_prefix} file send {current_directory+self.output_path[2:]} {hdc_dir}"
            result_push = subprocess.run(
                hdc_push, capture_output=True, text=True, shell=True
            )
            print(result_push)
        ...
        for run_config in run_configs:
            ...
            if "Success" in result.stdout:
                ...
                raise RuntimeError(f"run mode fail, because {result.stdout}")
            ...
            if hdc_dir is not None and hdc_shell_prefix is not None:
                ...
                gold_in_param_hdc = ""
                for i in range(input_length):
                    gold_in_param_hdc += golden_path_hdc + "/" + f"input_{i}.bin,"
                gold_out_param_hdc = golden_path_hdc + "/model.onnx.out"
                hdc_benchmark_path = hdc_dir + "/" + "benchmark_bin"
                hdc_command = f'{hdc_shell_prefix} shell "'
                new_input_shapes = input_shapes.replace(";", r"\;")
                args_hdc = [
                    hdc_benchmark_path,
                    f"--modelFile={in_model_path_hdc}",
                    "--device=CPU",
                    "--loopCount=1",
                    "--warmUpLoopCount=1",
                    f"--inDataFile={gold_in_param_hdc[:-1]}",
                    f"--benchmarkDataFile={gold_out_param_hdc}",
                    f"--inputShape={new_input_shapes}",
                ]
                for one_arg in args_hdc:
                    hdc_command += one_arg
                    hdc_command += " "
                hdc_command = hdc_command[:-1] + '"'
                print(hdc_command)
                result_hdc = subprocess.run(
                    hdc_command, capture_output=True, text=True, shell=True
                )
                if "Success" in result_hdc.stdout:
                    conf.logger.info(
                        "run model %s hdc infer success", run_config["in_model"]
                    )
                    self.num_success += 1
                else:
                    conf.logger.error(
                        "run model %s hdc infer failed", run_config["in_model"]
                    )
                    raise RuntimeError(f"run mode fail, because {result_hdc.stdout}")

    def exec_st(self):
        if self.model_configs.disable_all:
            conf.logger.info(
                "diasabled test: %s", self.model_configs.op_configs_["op_name"]
            )
        else:
            self.gen_onnx()
            self.gen_gold()
