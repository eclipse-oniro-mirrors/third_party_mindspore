...
int NetTrainCApi::SaveModels() {
  if (!flags_->export_file_.empty()) {
    if (flags_->bb_model_file_.empty()) {
      auto status = OH_AI_ExportModel(ms_model_, OH_AI_MODELTYPE_MINDIR, (flags_->export_file_ + "_qt").c_str(),
                                  OH_AI_WEIGHT_QUANT, false, nullptr, 0);
    ...
    auto status =
      OH_AI_ExportModel(ms_model_, OH_AI_MODELTYPE_MINDIR, (flags_->export_file_).c_str(), OH_AI_NO_QUANT, false, nullptr, 0);
    if (status != OH_AI_STATUS_SUCCESS) {
  ...
  if (!flags_->inference_file_.empty()) {
    auto status = OH_AI_ExportModel(ms_model_, OH_AI_MODELTYPE_MINDIR, (flags_->inference_file_ + "_qt").c_str(),
                                OH_AI_WEIGHT_QUANT, true, nullptr, 0);
    if (status != OH_AI_STATUS_SUCCESS) {
    ...
    auto tick = GetTimeUs();
    status =
      OH_AI_ExportModel(ms_model_, OH_AI_MODELTYPE_MINDIR, (flags_->inference_file_).c_str(), OH_AI_NO_QUANT, true, nullptr, 0);
    if (status != OH_AI_STATUS_SUCCESS) {
...
int NetTrainCApi::CreateAndRunNetworkForInference(const std::string &filename, const OH_AI_ContextHandle &context) {
  ...
  auto status =
    OH_AI_ModelBuildFromFile(ms_model_, filenamems.c_str(), static_cast<OH_AI_ModelType>(mindspore::kMindIR), context);
  if (status != OH_AI_STATUS_SUCCESS) {
...
int NetTrainCApi::CreateAndRunNetworkForTrain(const std::string &filename, const std::string &bb_filename,
                                              const OH_AI_ContextHandle &context, const OH_AI_TrainCfgHandle &train_cfg,
                                              int epochs) {
  std::string model_name = filename.substr(filename.find_last_of(DELIM_SLASH) + 1);
  OH_AI_Status status;
