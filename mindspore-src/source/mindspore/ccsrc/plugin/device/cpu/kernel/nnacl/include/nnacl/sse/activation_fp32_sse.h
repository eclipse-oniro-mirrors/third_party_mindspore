...
static inline int Fp32ClipSSE(int index, const float *src, int length, float *dst, float min, float max) {
...
static inline int Int32ClipSSE(int index, const int32_t *src, int length, int32_t *dst, int min, int max) {
...
static inline int SoftplusSSE(int index, const float *src, int length, float *dst) {
...
static inline int GeluTanhApproximateSSE(int index, const float *src, int length, float *dst) {
    for (int block_max_size = length - BLOCK_NUM + 1; index < block_max_size; index += BLOCK_NUM) {
...
static inline SIMD_F32 SIMD_ERFCCHEBSSE(SIMD_F32 src) {
...
static inline SIMD_F32 SIMD_ERF_APPROXIMATESSE(SIMD_F32 src) {
  SIMD_F32 abs_src = SIMD_ABS_F32(src);
  SIMD_F32 sign = SIMD_GETSIGN_F32(src);
  SIMD_F32 dst = SIMD_ERFCCHEBSSE(abs_src);
  return SIMD_MUL_F32(sign, SIMD_SUB_F32(SIMD_MOV_F32(1.0f), dst));
}

static inline int GeluErfAPPROXIMATESSE(int index, const float *src, int length, float *dst) {
    SIMD_F32 para1 = SIMD_MOV_F32(1.4142135623730951f);
    SIMD_F32 para2 = SIMD_MOV_F32(1.0f);
    SIMD_F32 para3 = SIMD_MOV_F32(0.5f);
    for (int block_max_size = length - BLOCK_NUM + 1; index < block_max_size; index += BLOCK_NUM) {
      SIMD_F32 in = SIMD_LD_F32(src + index);
      SIMD_F32 res = SIMD_MUL_F32(SIMD_MUL_F32(para3, in), SIMD_ADD_F32(para2, SIMD_ERF_APPROXIMATESSE(SIMD_DIV_F32(in, para1))));
      SIMD_ST_F32(dst + index, res);
    }
    return index;
}
...
static inline int HardShrinkSSE(int index, const float *src, int length, float *dst, float lambd) {
