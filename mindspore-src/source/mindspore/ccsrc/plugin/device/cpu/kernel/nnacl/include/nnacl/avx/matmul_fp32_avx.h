...
// act_type must be 0, 1, 2. 0: no_act, 1: relu, 3: relu6.
static inline int64_t Row1Deep1GemmIsNotPackAVX(int64_t index, const float *a, const float *b, float *c, const float *bias, int col,
  int act_type) {
  SIMD_F32 down_threshold = SIMD_MOV_F32(0.0f);
  SIMD_F32 up_threshold = SIMD_MOV_F32(6);
...
// act_type must be 0, 1, 2. 0: no_act, 1: relu, 3: relu6.
static inline int64_t Row1Deep1NoBiasGemmIsNotPackAVX(int64_t index, const float *a, const float *b, float *c, const float *bias, int col,
  int act_type) {
  SIMD_F32 down_threshold = SIMD_MOV_F32(0.0f);
  SIMD_F32 up_threshold = SIMD_MOV_F32(6);
  SIMD_F32 vec_a = SIMD_MOV_F32(a[0]);
